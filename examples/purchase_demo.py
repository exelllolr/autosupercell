...
При блокировке входа Supercell (unusual activity) API автоматически делает до 30 попыток:
каждый раз помечает текущий прокси как неудачный, запускает браузер с новым прокси и повторяет авторизацию.
        """.strip(),
    )
